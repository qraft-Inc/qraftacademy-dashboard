...
import EditMarketingModal from "./modals/EditMarketingModal";
import useShow from "./hooks/useShow";
import ViewMarketingModal from "./modals/ViewMarketingModal";
import PasswordModal from "./modals/PasswordModal";
import Image from "next/image";
import profile from "../public/images/profile.png";

export default function MarketingList({ user }) {
    ...
    <>
      <div className="sm:w-max p-1 flex gap-1 items-center rounded-lg border bg-white shadow-md md:flex-row hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
        ...
        <ul className="text-sm font-medium text-gray-900 rounded-lg border border-gray-200 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
          ...
          <li className="py-2 px-4 w-full border-b border-gray-200 dark:border-gray-600">
            Mobile : <span className="font-bold">{user.marketing.telephone}</span>
          </li>
        </ul>
      ...
      {showEditModal ? (
        <EditMarketingModal user={user} setEditModal={setEditModal} />
      ) : null}
      {showViewModal ? (
        <ViewMarketingModal user={user} setViewModal={setViewModal} />
      ) : null}
    </>
  );
}
